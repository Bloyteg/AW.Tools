...
namespace MrByte.ActionCommand.Parser.Test
open NUnit.Framework
open MrByte.FSharp.TestHelper.NUnitHelpers
open MrByte.ActionCommand.Parser.ParserHelpers
open FParsec

[<TestFixture>]
module ParserHelpersTests =
    let internal fail = MrByte.FSharp.TestHelper.NUnitHelpers.fail;
    ...
    let ``unorderedOpt2 returns two items that exist`` () =
        ...
        | Success(result, _, _) -> match result with
                                   ...
                                   | _ -> MrByte.FSharp.TestHelper.NUnitHelpers.fail()
        | _ -> MrByte.FSharp.TestHelper.NUnitHelpers.fail()
    ...
    let ``parse valid uri`` () =
        ...
        | Success(result, _, _) -> if result.AbsoluteUri = "http://test.com/" then pass() else MrByte.FSharp.TestHelper.NUnitHelpers.fail()
        | _ -> MrByte.FSharp.TestHelper.NUnitHelpers.fail()
    ...
    let ``parse valid uri with query string`` () =
        ...
        | Success(result, _, _) -> if result.AbsoluteUri = "http://test.com/?x=5&q=ss%20&t=5eB34" then pass() else MrByte.FSharp.TestHelper.NUnitHelpers.fail()
        | _ -> MrByte.FSharp.TestHelper.NUnitHelpers.fail()

    [<Test>]
    let ``parse invalid url`` () =
        match run (puri) "name=test.com" with
        | Success(_, _, _) -> MrByte.FSharp.TestHelper.NUnitHelpers.fail()
        | _ -> pass()
